...
/**
 * Композиция итоговой skin-текстуры игрока: {@code original skin + clothing textures = final skin}.
 * <p>
 * Реализует ровно алгоритм из ТЗ:
 * <ol>
 *     <li>всегда начинаем с СВЕЖЕЙ копии оригинального скина (никогда не мутируем накопительно -
 *     см. {@link #compose});</li>
 *     <li>слои одежды накладываются по возрастанию priority (нижний слой - первым);</li>
 *     <li>у слоя одежды меняются только пиксели с alpha &gt; 0;</li>
 *     <li>если слой одежды перекрывает пиксель ПЕРВОГО (базового) слоя скина, соответствующий
 *     пиксель ВТОРОГО слоя (hat/jacket/sleeve/pants overlay) автоматически обнуляется
 *     (alpha = 0), чтобы он не "просвечивал" поверх одежды.</li>
 * </ol>
 * Раскладка UV 64x64 skin-формата Minecraft (стабильна с версии 1.8, не связана с версией
 * Curios/NeoForge): для каждой из 6 частей тела прямоугольник первого слоя связан с
 * прямоугольником второго слоя фиксированным смещением - см. {@link #LAYER_PAIRS}.
 */
